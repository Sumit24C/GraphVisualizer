...
// nodeStates: { [id]: string } OR { [id]: { state: string, dist?: number, cost?: number } }
// edgeStates: { [id]: string }
export function drawGraph(canvas, nodes, edges, nodeStates = {}, edgeStates = {}) {
    if (!canvas) return;
    ...
    edges.forEach(e => {
        ...
        const lw = rawState !== "default" ? 2.5 : 1.5;
        ...
        ctx.lineWidth = lw;
        ctx.shadowColor = rawState !== "default" ? color : "transparent";
        ctx.shadowBlur = rawState !== "default" ? 8 : 0;
        ...
        ctx.shadowBlur = 0;
        ctx.fillStyle = rawState !== "default" ? color : "#1e293b";
        ctx.beginPath();
        ctx.roundRect(mx - pw / 2 + 6, my - ph / 2 - 6, pw, ph, 4);
        ctx.fill();

        ctx.fillStyle = rawState !== "default" ? "#0f172a" : "#94a3b8";
    ...
    nodes.forEach(n => {
        ...
        const stateKey = typeof raw === "object" ? (raw?.state ?? "default") : (raw ?? "default");
        const extra = typeof raw === "object" ? raw : null;   // { dist, cost, ... }

        const col = NODE_COLORS[stateKey] ?? NODE_COLORS.default;

        ctx.save();
        ctx.shadowColor = col.stroke;
        ctx.shadowBlur = stateKey !== "default" ? 14 : 4;
